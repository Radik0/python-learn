#!/usr/bin/python3
# -*- coding: utf-8 -*-
# @Time    : 2021/6/12 21:06
# @Author  : Huang
# @FileName: 维吉利亚密码.py
# @Software: PyCharm
# https://blog.csdn.net/SFS_Ccjm/article/details/105816072
# 实现列表对应元素相加（加密部分）
def hgl_1add_list(x, y):
    result = []
...
def hgl_05encode(s, key):
    ...
    for i in list_key:
        if len(list_key) <= len(list_s):
            list_key.append(i)
    list_result = hgl_1add_list(list_s, list_key)
...
def hgl_06decode(s, key):
    ...
    for i in list_key:
        if len(list_key) < len(list_s):
            list_key.append(i)
    list_result = hgl_02sub_list(list_s, list_key)
    for i in list_result:
        if i < 0:
            i += 26
        list_finall.append(dic_n2c[i])
