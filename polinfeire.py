#!/usr/bin/python3
# -*- coding: utf-8 -*-
# @Time    : 2021/6/12 21:18
# @Author  : Huang
# @FileName: polinfeire.py
# @Software: PyCharm
#########################Playfair密码#########################
...
# 根据密钥建立密码表
def hgl01_Create_Matrix(key):
    key = hgl_02Remove_Duplicates(key)  # 移除密钥中的重复字母
...
# 移除字符串中重复的字母
def hgl_02Remove_Duplicates(key):
...
# 加密
def hgl_04Encrypt(plaintext, T_letter):
    ciphertext = ''
            ...
            while j < len(plaintext):  # 直到遍历到字母，进行加密
                if True == plaintext[j].isalpha():
                    if 'I' == plaintext[i].upper():  #
                        x = hgl_03Get_MatrixIndex('J')  #
                    else:  #
                        x = hgl_03Get_MatrixIndex(plaintext[i].upper())  # 对字符在密码表中的坐标
                    if 'I' == plaintext[j].upper():  # 进行定位,同时将'I'作为
                        y = hgl_03Get_MatrixIndex('J')  # 'J'来处理
                    else:  #
                        y = hgl_03Get_MatrixIndex(plaintext[j].upper())  #
...
# 解密
def hgl_05Decrypt(ciphertext, T_letter):
    plaintext = ''
            ...
            while j < len(ciphertext):  # 直到遍历到字母，进行解密
                if True == ciphertext[j].isalpha():
                    if 'I' == ciphertext[i].upper():  #
                        ...
                        x = hgl_03Get_MatrixIndex(ciphertext[i].upper())  # 对字符在密码表中的坐标
                    if 'I' == ciphertext[j].upper():  # 进行定位,同时将'I'作为
                        y = hgl_03Get_MatrixIndex('J')  # 'J'来处理
                    else:  #
                        y = hgl_03Get_MatrixIndex(ciphertext[j].upper())  #
...
if __name__ == '__main__':
    ...
    hgl01_Create_Matrix(key)  # 建立密码表

    if user_input == 'D':  # 加密
        print('请输入明文:')
        plaintext = input()
        print("密文为:\n%s" % hgl_04Encrypt(plaintext, T_letter))
    else:  # 解密
        print('请输入密文:')
        ciphertext = input()
        print('明文为:\n%s' % hgl_05Decrypt(ciphertext, T_letter))
